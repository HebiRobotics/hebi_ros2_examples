...
def create_mobile_io_from_config(config, lookup, node):
...
def create_gripper_from_config(config, lookup, arm, node):
    ...
    if set(required_keys).issubset(config.user_data.keys()):
        ...
        # Check that all required fields are present and are non-empty strings
        if all(isinstance(config.user_data[key], str) for key in required_keys_str) and all(
            isinstance(config.user_data[key], float) for key in required_keys_float
        ):
